...
int BotGui::onKeyEvent(vx_layer_t* layer, vx_key_event_t* event)
{
    ...
    if(event->key_code == VX_KEY_UP)
    {
        ...
        lcmInstance_->publish(MAEBOT_MOTOR_COMMAND_CHANNEL, &cmd);
    }
...
void BotGui::onDisplayStart(vx_display_t* display)
{
    ...
    lcmInstance_->subscribe(RPLIDAR_LASER_CHANNEL, &BotGui::handleLaser, this);
...
void BotGui::handleLaser(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const rplidar_laser_t* laser)
{
    std::lock_guard<std::mutex> autoLock(vxLock_);
    laser_ = *laser;
    haveLaser_ = true;
...
//     gtk_container_add(GTK_CONTAINER(window), mainPane);
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(showDistancesCheck_), TRUE);
